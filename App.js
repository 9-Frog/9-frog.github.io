...
const colorOptions = [
    { key: 'oyster-white', name: 'Off White', ral: 'RAL1013', swatch: '#E9E5D0', frame: '#d9d5bf' },
    { key: 'anthracite-grey', name: 'Anthracite Grey', ral: 'RAL7016', swatch: '#2C3033', frame: '#394044' },
    { key: 'jet-black', name: 'Jet Black', ral: 'RAL9005', swatch: '#101214', frame: '#171b1d' },
    { key: 'pure-white', name: 'Pure White', ral: 'RAL9010', swatch: '#F5F5F2', frame: '#efefea' },
    { key: 'traffic-white', name: 'Traffic White', ral: 'RAL9016', swatch: '#FFFFFF', frame: '#f7f8f6' },
];
...
const roofLabelMap = {
    ...
    fabric: 'Fabric Pergola',
    bioclimatic: 'Bioclimatic Pergola',
    'lux-bioclimatic': 'Luxury Bioclimatic Pergola',
};
...
function loadImageElement(src) {
    return new Promise((resolve, reject) => {
        ...
        image.onerror = () => reject(new Error('The Visora watermark could not be loaded.'));
        image.src = src;
    });
}
...
async function captureWatermarkedSceneBlob(scene) {
    ...
    if (!context) {
        throw new Error('The export canvas could not be prepared.');
    }
    ...
    if (!blob) {
        throw new Error('The photo export could not be rendered.');
    }
    return blob;
}
async function createWatermarkedRecordingSurface(sourceCanvas) {
    ...
    if (!context) {
        throw new Error('The video recorder canvas could not be prepared.');
    }
    let frameId = 0;
    let running = true;
    const render = () => {
...
function loadModelViewer() {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (existing) {
            ...
            existing.addEventListener('error', () => reject(new Error('AR view could not be loaded.')), { once: true });
            return;
        }
        ...
        script.onerror = () => reject(new Error('AR view could not be loaded.'));
...
function ArQrModal({ open, onClose, shareUrl }) {
    ...
    return (_jsx("div", { className: "fixed inset-0 z-[80] flex items-center justify-center bg-[rgba(10,14,24,0.62)] px-4 py-6 backdrop-blur-[10px]", onClick: onClose, children: _jsxs("div", { className: "w-full max-w-[420px] rounded-[28px] border border-white/15 bg-[linear-gradient(180deg,rgba(19,25,39,0.98)_0%,rgba(12,16,26,0.96)_100%)] p-5 text-white shadow-[0_30px_80px_rgba(0,0,0,0.45)]", onClick: (event) => event.stopPropagation(), children: [_jsxs("div", { className: "flex items-start justify-between gap-4", children: [_jsxs("div", { children: [_jsx("div", { className: "text-[11px] uppercase tracking-[0.18em] text-white/45", children: "View on Phone" }), _jsx("h2", { className: "mt-2 text-[24px] font-semibold tracking-[-0.03em]", children: "Scan with Your Phone" }), _jsx("p", { className: "mt-2 text-[13px] leading-6 text-white/68", children: "Open the same project on your phone and launch the camera view from there." })] }), _jsx("button", { type: "button", onClick: onClose, className: "flex h-10 w-10 items-center justify-center rounded-full border border-white/12 bg-white/6 text-white/72 transition-all hover:bg-white/10 hover:text-white", "aria-label": "QR penceresini kapat", children: _jsx(X, { className: "h-4 w-4" }) })] }), _jsx("div", { className: "mt-5 rounded-[24px] bg-white p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.3)]", children: _jsx("img", { src: qrUrl, alt: "QR code for phone view", className: "mx-auto block h-[280px] w-[280px] rounded-[18px]" }) }), _jsx("div", { className: "mt-4 rounded-[18px] border border-white/10 bg-white/5 px-4 py-3 text-[12px] leading-5 text-white/60", children: "For the best result, open the site on your phone. The QR code will work once the site is published online." })] }) }));
}
...
function ArExperiencePage() {
    ...
    useEffect(() => {
        ...
        loadModelViewer()
            ...
            .catch((loadError) => {
            if (!cancelled)
                setViewerError(loadError instanceof Error ? loadError.message : 'AR view could not be loaded.');
        });
        return () => {
            cancelled = true;
        };
    ...
    useEffect(() => {
        ...
        exportConfigToGlb(arConfig)
            ...
            .catch((exportError) => {
            if (!cancelled)
                setViewerError(exportError instanceof Error ? exportError.message : 'The AR model could not be prepared.');
        });
        return () => {
            cancelled = true;
            if (nextUrl)
    ...
    useEffect(() => {
        ...
        const startCamera = async () => {
            if (!canUseLiveCamera()) {
                setCameraError('This browser cannot show a live preview here. The phone view is still available.');
                return;
            }
            ...
            if (!stream) {
                setCameraError(lastError instanceof Error ? lastError.message : 'Please allow camera access for the live preview.');
                return;
            }
            if (!active) {
...
function AnimateControlButton({ active, isMobile, mobileMenuOpen, menuSections, onToggleAll, onToggleMobileMenu, }) {
    ...
    if (isMobile) {
        return (_jsx("button", { type: "button", "aria-label": mobileMenuOpen ? 'Close animation menu' : 'Open animation menu', onClick: onToggleMobileMenu, className: `flex h-10 w-10 items-center justify-center rounded-full border backdrop-blur-[8px] transition-colors ${mobileMenuOpen || active
                ? 'border-black bg-black text-white hover:bg-[#002f2b]'
...
function DownloadControlButton({ active, busy, isMobile, mobileMenuOpen, items, onToggleMobileMenu, }) {
    ...
    if (isMobile) {
        return (_jsx("button", { type: "button", "aria-label": mobileMenuOpen ? 'Close download menu' : 'Open download menu', onClick: onToggleMobileMenu, disabled: busy, className: `flex h-10 w-10 items-center justify-center rounded-full border backdrop-blur-[8px] transition-colors ${mobileMenuOpen || active
                ? 'border-black bg-black text-white hover:bg-[#002f2b]'
...
function ComingSoonModal({ open, title, onClose, theme }) {
    ...
    return (_jsx("div", { className: "fixed inset-0 z-[120] flex items-center justify-center px-4 py-6 backdrop-blur-[12px]", style: { background: theme.modalOverlay }, onClick: onClose, children: _jsxs("div", { className: "relative w-full max-w-[460px] rounded-[30px] border p-6 shadow-[0_30px_80px_rgba(0,0,0,0.35)]", style: {
                ...
                color: theme.text,
                        ...
                        color: theme.accent,
                    }, children: "Coming Soon" }), _jsx("h2", { className: "mt-4 text-[28px] font-semibold tracking-[-0.04em]", children: "Launching Soon" }), _jsxs("p", { className: "mt-3 text-[14px] leading-7", style: { color: mixHex(theme.text, '#ffffff', 0.18) }, children: [_jsx("span", { className: "font-semibold", style: { color: theme.text }, children: title }), " configurator is getting a modern demo screen. This section will be available soon."] })] }) }));
}
...
function ConfiguratorApp() {
    ...
    const [roofMaterial, setRoofMaterial] = useState(defaultRoofMaterial);
    const [pergolaColorOverride, setPergolaColorOverride] = useState(null);
    ...
    const [roofAwningEnabled, setRoofAwningEnabled] = useState(false);
    const [sideAwnings, setSideAwnings] = useState({
        front: false,
        frontLeft: false,
        frontRight: false,
        left: false,
    ...
    const [awningColor, setAwningColor] = useState('anthracite-grey');
    const [lightTemperature, setLightTemperature] = useState('warm');
    const [nightMode, setNightMode] = useState(false);
    const [roofAnimationOpen, setRoofAnimationOpen] = useState(false);
    const [doorAnimationOpen, setDoorAnimationOpen] = useState({
        front: false,
        frontLeft: false,
        frontRight: false,
        left: false,
    ...
    const [awningAnimationOpen, setAwningAnimationOpen] = useState({
        roof: false,
        front: false,
        frontLeft: false,
        frontRight: false,
    ...
    const playExportAwningSequence = async (keys, detail) => {
        ...
        if (!availableKeys.length)
            return false;
        setExportStatus({ title: 'Recording video', detail });
    ...
    const playExportSideSequence = async ({ view, awningKeys, doorKeys, openingCurtainDetail, openingSideDetail, }) => {
        ...
        if (curtainOpened) {
            await closeExportAwningSequence(view === 'front' ? 'Closing the front curtain...' : `Closing the ${view} curtain...`);
        }
    ...
    const handlePhotoExport = async () => {
        ...
        if (!scene || exportStatus)
            return;
        setMobileDownloadMenuOpen(false);
        setExportStatus({ title: 'Preparing photo', detail: 'Capturing 5 separate angles...' });
        ...
        try {
            for (let index = 0; index < EXPORT_PHOTO_VIEWS.length; index += 1) {
                ...
                setExportStatus({
                    ...
                    detail: `Capturing angle ${index + 1} of ${EXPORT_PHOTO_VIEWS.length}...`,
                });
            ...
            console.error(error);
            window.alert(error instanceof Error ? error.message : 'The photo export could not be created.');
        }
        finally {
    ...
    const handleVideoExport = async () => {
        ...
        const sourceCanvas = scene.getCanvas();
        if (!sourceCanvas) {
            window.alert('The scene video stream is not ready yet.');
            return;
        }
        ...
        const stopPromise = new Promise((resolve, reject) => {
            ...
            recorder.onerror = () => reject(new Error('The video export failed to record.'));
            recorder.onstop = () => resolve(new Blob(chunks, { type: mimeType ?? 'video/webm' }));
        });
        setMobileDownloadMenuOpen(false);
        setExportStatus({ title: 'Preparing video', detail: 'Resetting the animated parts...' });
        try {
            ...
            if (animationItemCount <= 0) {
                setExportStatus({ title: 'Recording video', detail: 'Capturing the scene walkthrough...' });
                ...
                await playExportSideSequence({
                    ...
                    awningKeys: ['right'],
                    doorKeys: ['right'],
                    openingCurtainDetail: 'Opening the right curtain...',
                    openingSideDetail: 'Opening the right side...',
                });
                await playExportSideSequence({
                    view: 'front',
                    awningKeys: frontVideoSides,
                    doorKeys: frontVideoSides,
                    openingCurtainDetail: 'Opening the front curtain...',
                    openingSideDetail: 'Opening the front side...',
                });
                await playExportSideSequence({
                    view: 'left',
                    awningKeys: ['left'],
                    doorKeys: ['left'],
                    openingCurtainDetail: 'Opening the left curtain...',
                    openingSideDetail: 'Opening the left side...',
                });
                await scene.moveToView('top', { durationMs: 1500 });
                await sleep(220);
                const roofCurtainOpened = await playExportAwningSequence(['roof'], 'Opening the roof curtain...');
                if (roofCurtainOpened) {
                    await sleep(260);
                }
                await playExportRoofSequence('Opening the roof system...');
                if (roofCurtainOpened) {
                    await closeExportAwningSequence('Closing the roof curtain...');
                }
                if (!roofHasAction)
                    await sleep(700);
            ...
            triggerBlobDownload(videoBlob, `visora-video-${timestamp}.${extension}`);
        }
        catch (error) {
            console.error(error);
            ...
            window.alert(error instanceof Error ? error.message : 'The video export could not be created.');
        }
        finally {
    ...
    const inquiryText = useMemo(() => {
        const lines = [
            `Hello ${siteConfig.branding.whatsappBrandName},`,
            '',
            'Contact information',
            `• Name: ${inquiryForm.name.trim() || 'Not specified'}`,
            `• Post code: ${inquiryForm.postcode.trim() || 'Not specified'}`,
            `• Email: ${inquiryForm.email.trim() || 'Not specified'}`,
            ...(inquiryForm.note.trim() ? ['', 'Additional note', inquiryForm.note.trim()] : []),
            '',
            `I would like to receive pricing information for the following ${siteConfig.branding.whatsappBrandName} configuration:`,
            '',
            'Dimensions',
            `• Width: ${formatMetres(dimensions.width)}`,
            `• Depth: ${formatMetres(dimensions.projection)}`,
            `• Height: ${formatMetres(dimensions.height)}`,
            '',
            'Features',
            `• Roof: ${roofMaterialLabel(roofMaterial)}`,
            `• Frame color: ${activeColor.name} (${activeColor.ral})`,
            ...(isPergolaRoof ? [`• Pergola color: ${pergolaColor.name} (${pergolaColor.ral})`] : []),
            ...(frontIsSplit
                ? [
                    `• Front left side: ${enclosureSummary(enclosures.frontLeft)}`,
                    `• Front right side: ${enclosureSummary(enclosures.frontRight)}`,
                ]
                : [`• Front side: ${enclosureSummary(enclosures.front)}`]),
            `• Left side: ${enclosureSummary(enclosures.left)}`,
            `• Right side: ${enclosureSummary(enclosures.right)}`,
            '• Top glazing: Glass',
            '',
            'Extras',
            `• Roof awning: ${roofAwningEnabled ? `Yes / ${awningActiveColor.name}` : 'No'}`,
            `• Front awning: ${frontIsSplit ? `Left ${sideAwnings.frontLeft ? 'Yes' : 'No'} / Right ${sideAwnings.frontRight ? 'Yes' : 'No'}` : sideAwnings.front ? 'Yes' : 'No'}`,
            `• Left awning: ${sideAwnings.left ? 'Yes' : 'No'}`,
            `• Right awning: ${sideAwnings.right ? 'Yes' : 'No'}`,
            `• Lights: ${lightsEnabled ? `Round LED lights / ${lightTemperature === 'warm' ? 'Warm White 3000K' : 'Neutral White 4000K'}` : 'Not selected'}`,
            '',
            'Could you share the estimated price, delivery time, and your recommendations?',
            '',
            'Thank you.',
        ];
        return lines.join('\n');
    ...
    const dimensionsContent = (_jsxs(_Fragment, { children: [_jsxs("div", { className: "grid grid-cols-3 gap-2.5", children: [_jsx(SummaryMetric, { value: formatMetres(dimensions.width), label: "Width" }), _jsx(SummaryMetric, { value: formatMetres(dimensions.projection), label: "Depth" }), _jsx(SummaryMetric, { value: formatMetres(dimensions.height), label: "Height" })] }), _jsxs("div", { className: "mt-4 space-y-4", children: [_jsx(SliderRow, { label: "Width", range: "Between 2 m and 7 m", value: dimensions.width, min: 2, max: 7, step: 0.1, onChange: (value) => setDimensions((prev) => ({ ...prev, width: Number(value.toFixed(1)) })) }), _jsx(SliderRow, { label: "Depth", range: "Between 2 m and 4 m", value: dimensions.projection, min: 2, max: 4, step: 0.1, onChange: (value) => setDimensions((prev) => ({ ...prev, projection: Number(value.toFixed(1)) })) }), _jsx(SliderRow, { label: "Height", range: "Between 2 m and 2.2 m", value: dimensions.height, min: 2, max: 2.2, step: 0.01, onChange: (value) => setDimensions((prev) => ({ ...prev, height: Number(value.toFixed(2)) })) })] })] }));
    const roofContent = (_jsxs(_Fragment, { children: [_jsx("div", { className: "grid grid-cols-2 gap-2.5", children: enabledRoofOptions.map((option) => (_jsx(PillButton, { active: roofMaterial === option.key, label: option.label, onClick: () => setRoofMaterial(option.key) }, option.key))) }), isPergolaRoof ? (_jsxs("div", { className: "mt-4 border-t border-[#c7d9d6] pt-4", children: [_jsxs("div", { className: "mb-2 flex items-center justify-between gap-3", children: [_jsxs("p", { className: "text-[12px] text-muted", children: ["Pergola color: ", _jsx("span", { className: "font-medium text-ink", children: pergolaColor.ral })] }), _jsx("button", { type: "button", onClick: () => setPergolaColorOverride(null), className: `rounded-full border px-3 py-1.5 text-[11px] font-medium transition-colors ${pergolaColorOverride === null
                                    ? 'border-[#0b6a60] bg-[#003934]/8 text-ink'
                                    : 'border-[#c7d9d6] bg-white text-muted hover:border-[#7c9894] hover:text-ink'}`, children: "Same as structure" })] }), _jsx("div", { className: "grid grid-cols-3 gap-2 sm:grid-cols-5", children: colorOptions.map((option) => (_jsx(Swatch, { option: option, active: option.key === (pergolaColorOverride ?? selectedColor), onClick: () => setPergolaColorOverride(option.key) }, `pergola-${option.key}`))) })] })) : null] }));
    const colorsContent = (_jsxs(_Fragment, { children: [_jsxs("p", { className: "mb-3 text-[12px] text-muted", children: ["Frame color: ", _jsx("span", { className: "font-medium text-ink", children: activeColor.ral })] }), _jsx("div", { className: "grid grid-cols-3 gap-2 sm:grid-cols-5", children: colorOptions.map((option) => (_jsx(Swatch, { option: option, active: option.key === selectedColor, onClick: () => setSelectedColor(option.key) }, option.key))) })] }));
    const enclosureContent = (_jsxs(_Fragment, { children: [_jsxs("div", { className: "space-y-1 rounded-[13px] border border-[#c7d9d6] bg-[linear-gradient(180deg,#FFFFFF_0%,#ffffff_100%)] px-3 py-3 text-[12px] text-muted", children: [frontIsSplit ? (_jsxs(_Fragment, { children: [_jsxs("div", { children: ["Front left: ", _jsx("span", { className: "font-medium text-ink", children: enclosureSummary(enclosures.frontLeft) })] }), _jsxs("div", { children: ["Front right: ", _jsx("span", { className: "font-medium text-ink", children: enclosureSummary(enclosures.frontRight) })] })] })) : (_jsxs("div", { children: ["Front: ", _jsx("span", { className: "font-medium text-ink", children: enclosureSummary(enclosures.front) })] })), _jsxs("div", { children: ["Left: ", _jsx("span", { className: "font-medium text-ink", children: enclosureSummary(enclosures.left) })] }), _jsxs("div", { children: ["Right: ", _jsx("span", { className: "font-medium text-ink", children: enclosureSummary(enclosures.right) })] })] }), _jsx("div", { className: `mt-4 grid gap-2.5 ${frontIsSplit ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-3'}`, children: (frontIsSplit ? ['frontLeft', 'frontRight', 'left', 'right'] : ['front', 'left', 'right']).map((side) => (_jsxs("button", { type: "button", onClick: () => {
                        if (frontIsSplit &&
    ...
    const downloadMenuItems = [
        {
            key: 'photo',
            label: 'As a photo',
            detail: 'Save 5 separate images from different 3D angles.',
        ...
        },
        {
            key: 'video',
            label: 'As a video',
            detail: 'Record a walkthrough with the animated parts opening.',
    ...
    const mobileAnimationContent = (_jsxs(Card, { className: "border-[#c7d9d6] bg-white/95 shadow-[0_20px_44px_rgba(37,44,49,0.08)]", children: [_jsx(SectionTitle, { helper: "Open or close the roof, sides and curtains one by one.", children: "Animation" }), _jsx(AnimateMenuPanel, { sections: animationSections })] }));
    const mobileDownloadContent = (_jsxs(Card, { className: "border-[#c7d9d6] bg-white/95 shadow-[0_20px_44px_rgba(37,44,49,0.08)]", children: [_jsx(SectionTitle, { helper: "Save 5 separate photos or a short animated walkthrough.", children: "Download" }), _jsx(DownloadMenuPanel, { items: downloadMenuItems })] }));
    const mobileSections = {
        dimensions: {
            title: 'Dimensions',
            helper: 'Use the sliders or enter the values manually.',
            content: dimensionsContent,
        },
        roof: {
            title: 'Roof',
            helper: 'Choose the roof type for your design.',
            content: roofContent,
        },
        enclosures: {
            title: 'Sides',
            helper: 'Choose a side and set the enclosure type.',
            content: enclosureContent,
        },
        colors: {
            title: 'Color',
            helper: 'Choose the frame color.',
        ...
        ...(hasAnyAddonOption
            ? {
                addons: {
                    title: 'Extras',
                    helper: 'Lighting and awning options.',
